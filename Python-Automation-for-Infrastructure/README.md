...
Contributing & license
- This module is intended as part of a larger Advanced-DevOps project. If you contribute, keep changes backwards-compatible and add tests for behavioral changes.
- Add a LICENSE file to the repository root if you intend to publish this module on GitHub with a specific license.

--

This README is intentionally concise but complete for a GitHub module page. If you want, I can next:
- Implement DB-backed task persistence and update Celery tasks to write results to the DB (recommended for production), or
- Add a small test suite that verifies the full end-to-end flow (create -> execute -> callback -> final status).
